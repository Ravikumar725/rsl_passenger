...
void calculateApproximateFareNew(
    List<CarModelData> fareDetailsList, TaxiControllerNew taxiControllerNew) {
  try {
    double distance = taxiControllerNew.approximateDistance.value;
    double time = taxiControllerNew.approximateTime.value;
    double trafficTime = taxiControllerNew.approximateTrafficTime.value;
    ...
    for (CarModelData fareDetails in fareDetailsList) {
      ...
      // Apply dynamic price hike (if available)
      if (fareDetails.priceHike != null &&
          fareDetails.priceHike!.contains('x')) {
        double priceHike = double.parse(
            fareDetails.priceHike!.replaceAll('x', '')); // Parse the multiplier
        approximateFareMax = approximateFareMax * priceHike;
      }
      ...
      taxiControllerNew.carModelList
        ..forEach((item) {
    ...
    taxiControllerNew.distanceCalculated.value =
...
void calculateApproximateFareNewF(
    List<CarModelData> fareDetailsList, TaxiControllerNew taxiControllerNew) {
  try {
    double distance = taxiControllerNew.approximateDistance.value;
    double time = taxiControllerNew.approximateTime.value;
